...
| [`hardware/`](hardware/) | 8-point FFT (C golden model + Verilog RTL + Makefile); [`phycv_pst_cuda.cu`](hardware/phycv_pst_cuda.cu) — CUDA port of phycv's Phase Stretch Transform, verified against [an independent Python reference](scripts/generate_pst_cuda_reference.py); [`mie_scattering/`](hardware/mie_scattering/) — single-sphere Mie scattering (BHMIE algorithm) in CUDA, verified against an independent Python reimplementation and scipy's own Bessel functions |
